...
amino_name_map = {
    ...
    "고등어 (Mackerel)":             None,
    "굴 (Oyster, 익힘)":             None,
    "초록홍합 (Green-Lipped Mussel, 익힘)":          None,
}
...
omega_db = {
    ...
    "굴 (Oyster, 익힘)":       (0.034,0.812,"0.04:1","USDA FDC 171980 oyster eastern wild cooked"),
    "초록홍합 (Green-Lipped Mussel, 익힘)":    (0.18, 1.1,  "0.16:1","FSANZ NZ green-lipped mussel cooked 추정"),
}
...
""", unsafe_allow_html=True)
    ...
    if c_selected:
        ...
        for f in c_selected:
            ...
            with fc1:
                raw_g = st.number_input(f"{f} 생고기 (g)", 0, 1000, 50, step=5, key=f"craw_{f}")
            ...
            with fc3:
                ...
                use_actual = st.checkbox("실제 무게 직접 입력", key=f"cactual_chk_{f}")
            with fc4:
                if use_actual:
    ...
    if c_selected:
        ...
        for f in c_selected:
            ...
            ak = amino_name_map.get(f)
            if ak and ak in amino_db:
                rf_aa = get_retention_factor("단백질(g)", cooking_method) if cat_f != 'veggie' else 1.0
                for aa, val in amino_db[ak].items():
            ...
            if f in omega_db:
                ...
                rf_o3 = get_retention_factor("오메가3", cooking_method) if cat_f != 'veggie' else 1.0
                c_omega6 += o6 * ratio
