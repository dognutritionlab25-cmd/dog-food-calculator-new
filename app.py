...
st.set_page_config(page_title="반려견 영양 연구소 계산기 v5.2", layout="wide")
st.title("🐶 반려견 영양 연구소 [생식 계산기 v5.2]")
st.info("💡 전문가용 맞춤형 영양 컨설팅 & 레시피 분석 시스템 | 내장육 분류 수정 · 신규 12종 추가 (소신장·소비장·닭간·닭심장·닭근위 등)")
    ...
    with tab1:
        ...
        with c2:
            ...
            if total_kcal > 0:
                ...
                if p > 0: st.info(f"🦴 **Ca:P 비율 = {ca/p:.2f} : 1** (권장 1.1~2 : 1)")
...
st.markdown("---")
st.caption("반려견영양연구소 | 생식 계산기 v5.2")
