...
st.set_page_config(page_title="반려견 영양 연구소 계산기 v6.1", layout="wide")
st.title("🐶 반려견 영양 연구소 [영양 계산기 v6.1]")
st.info("💡 전문가용 맞춤형 영양 컨설팅 & 레시피 분석 시스템 | v6.1: 뼈고기 칼로리/단백질/지방 살코기 비율 기준 재계산, 전체 메추리 껍질 포함/제거 분리")
    ...
    if selected:
        ...
        with tab1:
            ...
            with c2:
                ...
                if total_kcal > 0:
                    ...
                    st.dataframe(res_df.style.map(color_status, subset=['판정']), width="stretch")
                    if p > 0:
        ...
        with tab2:
            ...
            if has_amino and total_kcal > 0:
                ...
                def color_aa(val):
                    ...
                    return ""
                st.dataframe(pd.DataFrame(aa_result).style.map(color_aa,subset=["판정"]),width="stretch",hide_index=True)
        ...
        with tab3:
            ...
            if omega6_total + omega3_final > 0:
                ...
                if od: st.dataframe(pd.DataFrame(od),width="stretch")
                if supp_o3 > 0:
    ...
    if c_selected:
        ...
        with ctab1:
            ...
            with rc2:
                ...
                if c_total_kcal > 0:
                    ...
                    def c_color(val):
                        return f'color:{"green" if "적합" in val else "red" if "부족" in val else "orange"};font-weight:bold'
                    st.dataframe(res_df.style.map(c_color, subset=['판정']), width="stretch")
        ...
        with ctab3:
            ...
            if has_amino and c_total_kcal > 0:
                ...
                st.dataframe(pd.DataFrame(aa_display), width="stretch", hide_index=True)
            else:
