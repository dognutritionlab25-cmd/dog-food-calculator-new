...
st.set_page_config(page_title="반려견 영양 연구소 계산기 v5.1", layout="wide")
st.title("🐶 반려견 영양 연구소 [생식 계산기 v5.1]")
st.info("💡 전문가용 맞춤형 영양 컨설팅 & 레시피 분석 시스템 | 야채 9종 · 사슴고기 아미노산 · 아연:구리 비율 추가")
    ...
    with tab2:
        ...
        if has_amino and total_kcal > 0:
            ...
            with card2:
                ...
                st.metric("💪 BCAA 합계 (류신+이소류신+발린)", f"{bcaa_total:.0f} mg", delta=f"1000kcal당 {bcaa_p1k:.0f} mg")
                st.markdown("")
            ...
            with card3:
                ...
                st.metric("🟡 황함유 — 메티오닌", f"{met:.0f} mg", delta=f"1000kcal당 {met1k:.0f} mg")
                ...
                st.metric("🔵 AAA — 페닐알라닌 + 트립토판", f"{aaa:.0f} mg", delta=f"1000kcal당 {aaa1k:.0f} mg")
