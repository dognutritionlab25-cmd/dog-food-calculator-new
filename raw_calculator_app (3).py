...
st.set_page_config(page_title="반려견 영양 연구소 계산기 v5.6", layout="wide")
st.title("🐶 반려견 영양 연구소 [생식 계산기 v5.6]")
st.info("💡 전문가용 맞춤형 영양 컨설팅 & 레시피 분석 시스템 | v5.6: 뼈재료 칼슘 유사종 실측 평균 기반 추정값 교정 (가금류 30.6mg/g, 포유류 50.4mg/g)")
        ...
        with c2:
            ...
            if total_kcal > 0:
                ...
                for f in selected:
                    ...
                    if not rows.empty:
                        row_f = rows.iloc[0]
                        src = row_f.get('칼슘출처', '') if hasattr(row_f, 'get') else ''
                        if isinstance(src, str) and src.startswith('est') and row_f.get('category') == 'bone':
                            est_items.append(f)
